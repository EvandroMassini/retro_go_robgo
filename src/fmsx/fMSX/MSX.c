...
int ResetMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  if((Mode^NewMode)&MSX_MODEL)
  {
    ...
    switch(NewMode&MSX_MODEL)
    {
      case MSX_MSX1:
        ...
        P1=LoadROM("MSX2/MSX.ROM",0x8000,0);
        PRINTRESULT(P1);
      ...
      case MSX_MSX2:
        ...
        P1=LoadROM("MSX2/MSX2.ROM",0x8000,0);
        PRINTRESULT(P1);
        if(Verbose) printf("  Opening MSX2EXT.ROM...");
        P2=LoadROM("MSX2/MSX2EXT.ROM",0x4000,0);
        PRINTRESULT(P2);
        if(!P1||!P2)
        {
      ...
      case MSX_MSX2P:
        ...
        P1=LoadROM("MSX2/MSX2P.ROM",0x8000,0);
        PRINTRESULT(P1);
        if(Verbose) printf("  Opening MSX2PEXT.ROM...");
        P2=LoadROM("MSX2/MSX2PEXT.ROM",0x4000,0);
        PRINTRESULT(P2);
        if(!P1||!P2)
        {
          NewMode=(NewMode&~MSX_MODEL)|(Mode&MSX_MODEL);
  ...
  if((Mode^NewMode)&MSX_PATCHBDOS)
  {
    ...
    /* Try loading DiskROM */
    if(Verbose) printf("  Opening MSX2DISK.ROM...");
    P1=LoadROM("MSX2/DISK.ROM",0x4000,0);
    PRINTRESULT(P1);
