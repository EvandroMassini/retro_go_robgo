...
{
  /* When BG is transparent, do not show shadow at all */
  if(BG<0) { PrintXY(Img,S,X,Y,FG,BG);return; }

  /* Update shadow font as required */
  if(CurShadow!=CurFont)
  {
    ...
    for(J=sizeof(ShadowFont)>>3,SRC=CurFont,DST=ShadowFont;J;--J,SRC+=8,DST+=8)
    {
      DST[0] = SRC[0]|SRC[1];
      DST[1] = SRC[1]|SRC[2]|SRC[0];
      DST[2] = SRC[2]|SRC[3]|SRC[1];
      DST[3] = SRC[3]|SRC[4]|SRC[2];
      DST[4] = SRC[4]|SRC[5]|SRC[3];
      DST[5] = SRC[5]|SRC[6]|SRC[4];
      DST[6] = SRC[6]|SRC[7]|SRC[5];
      DST[7] = SRC[7]|SRC[6];
    }

    /* Make horizontal shadow */
    for(J=0;J<sizeof(ShadowFont);++J)
      ShadowFont[J] = ShadowFont[J]|(ShadowFont[J]<<1)|(ShadowFont[J]>>1);

    /* Shadow font updated */
    CurShadow = CurFont;
  }

  /* Print shadows, then actual letters */
  CONSetFont(ShadowFont);
  PrintXY(Img,S,X,Y,BG,-1);
  CONSetFont(CurShadow);
  PrintXY(Img,S,X,Y,FG,-1);
}
